...
extract_recent_context() {
  ...
  tail -200 "$transcript_path" 2>/dev/null | jq -sr "${_MEMORIES_TURN_TEXT_JQ:-}${_MEMORIES_QUERY_TEXT_JQ:-def query_text: .;}"'
    [
      .[]
      | select(.type == "user" or .type == "assistant")
      | {
          role: .type,
          text: (turn_text | query_text)
        }
...
if [ ${#QUERY_PROMPT} -lt 20 ] && [ -z "$CONTEXT" ]; then
  exit 0
fi
