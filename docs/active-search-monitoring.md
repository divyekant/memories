...
- automatic hook search count (`search_count`)
...
Codex emits `prompt_evaluated` for every non-empty prompt, including short
follow-ups and zero-candidate prompts, and emits `session_recall` for every
SessionStart recall pass. Both include `search_count`, so automatic retrieval
volume is auditable per session. Claude Code emits `prompt_evaluated` for every
active-search-required prompt and every prompt where candidate memories were
injected. Candidate ids appear in the local telemetry log only —
active-search-required hook context still never exposes memory ids to the
model.
...
This does not change the playbook behavior. `prompt_evaluated` events are
still emitted for every active-search-required prompt, and a required prompt with
...
- `prompt_evaluations`: prompt hook evaluations represented in the log.
- `session_recall_events`: Codex SessionStart recall passes represented in the log.
- `automatic_searches`: sum of automatic hook searches reported by
  `prompt_evaluated.search_count` and `session_recall.search_count`.
...
curl -s -H "X-API-Key: $MEMORIES_API_KEY" \
  "$MEMORIES_URL/usage?period=7d&session_id=<codex-session-id>" | jq '.operations.search'
...
Hook requests and the MCP bridge attach `X-Memories-Client`,
`X-Memories-Session-Id` when available, and `X-Memories-Invocation`. The usage
dashboard exposes these dimensions and accepts an optional session-id filter.
Codex hook searches are therefore counted as normal `/search` operations; an
unset caller source is retained as `(unknown)` instead of being hidden.
