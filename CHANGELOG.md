...
- **`uninstall` now removes the read-only permission allowlist.** Both the Claude Code and Codex adapters wrote allow-rules on install and never removed them, so every uninstall left them behind; `--mcp-name` widened the leak. Removal matches on the tool suffix rather than the server name, so it clears the rules any past `init --mcp-name` wrote without needing the same flag repeated. Write-tool rules, wildcards, and unrelated entries are left untouched, and an emptied `allow`/`permissions` container is pruned rather than left behind as `[]`/`{}`.
