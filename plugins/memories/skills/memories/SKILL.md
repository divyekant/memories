...
- **Session hooks and plugins** already provide baseline context where supported — Claude Code / Cursor use the full 12-hook lifecycle, Codex uses a version-aware ten/five native-hook profile plus MCP and developer instructions, and OpenCode uses MCP plus plugin prompt recall and memory-tool telemetry. Don't duplicate that work.
...
- **Extraction hooks** already trigger `memory_extract` via HTTP at lifecycle boundaries for clients that support them.
  Claude Code / Cursor use `Stop`, `PreCompact`, and `SessionEnd`; Codex's
  expanded profile adds `PreCompact`, `SubagentStop`, and `SessionEnd`, while
  its legacy profile retains the beefier `Stop` capture. OpenCode does not
  auto-extract by default yet; OpenCode extraction is gated until reliable
  end-of-turn transcript access is proven.
...
Codex setup is owned by the published npm installer. For local stdio, run
`npx -y memories-mcp@latest init --codex`; for a hosted endpoint, run
`npx -y memories-mcp@latest init --codex --mcp-url https://... --yes` and then
`codex mcp login memories`. No repository checkout is required for runtime setup.

The installer selects the lifecycle from the installed client version:
Codex `>= 0.146.0` receives ten events, while older or unparseable versions
receive the five-event legacy profile.

| Profile | Events |
|---------|--------|
| Expanded (Codex `>= 0.146.0`) | `SessionStart`, `UserPromptSubmit`, `Stop`, `PreToolUse`, `PostToolUse`, `PreCompact`, `PostCompact`, `SubagentStart`, `SubagentStop`, `SessionEnd` |
| Legacy (older/unknown) | `SessionStart`, `UserPromptSubmit`, `Stop`, `PreToolUse`, `PostToolUse` |

The expanded profile adds `PreCompact` extraction (`context=pre_compact`),
`SubagentStart` recall, `SubagentStop` extraction, and one `SessionEnd`
enqueue (`context=session_end`). `PostCompact` is intentionally silent with
`suppressOutput`; Codex rehydrates through `SessionStart(source=compact)`.
`SessionEnd` makes one first-routed request with a two-second max-time and no
queue polling; its hook timeout is exactly three seconds. The Stop hook keeps
the larger Codex transcript window (500 tail lines, 10 message pairs, 8000
characters, no signal filter).

Hooks are installed in `~/.codex/hooks.json` and scripts in
`~/.codex/hooks/memory/`. MCP registration, developer instructions, and the
current approval policy live in `~/.codex/config.toml`: six read-only tools are
auto-approved there, while mutating tools—including `memory_is_useful`—remain
prompt-gated. The installer does not write `~/.codex/settings.json`.

Codex native Memories is optional and remains a local derived cache. External
Memories MCP is the durable, searchable cross-client authority; the installer
does not force either mode. If duplicate native context is undesirable, set
`memories.disable_on_external_context = true` manually in `config.toml`.
Active-search monitoring writes privacy-safe local telemetry to
`~/.config/memories/active-search.jsonl`; summarize it with
`.venv/bin/python scripts/active_search_metrics.py --log ~/.config/memories/active-search.jsonl`.
...
Codex does not rely on a `MEMORY.md` hydration file. It uses
`memory-recall.sh`, `memory-query.sh`, the Memories MCP server, and developer
instructions; Codex's optional native Memories cache may coexist as a local
derived cache and is not configured by the installer.
